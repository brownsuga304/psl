...
public class GmkUtils {
    ...
    for (String server : servers) {
      ...
      if (hostname.contains("managedkafka") && hostname.contains(".cloud.goog")) {
        try {
          ...
          return ValidationResult.success("GMK bootstrap server resolved successfully: " + hostname);
        } catch (Exception e) {
          return ValidationResult.error(
              "Failed to resolve GMK hostname: " + hostname + ". " +
              "Please verify:\n" +
              ...
              "Use: gcloud managed-kafka clusters list --location=<region> --project=<project>"
          );
        }
      }
    }

    return ValidationResult.warning("Bootstrap servers don't appear to be GMK endpoints: " + bootstrapServers);
  }
   ...
   * @param port Port number (typically 9092 for GMK)
   * @return Formatted bootstrap server URL
   */
  public static String buildGmkBootstrapServer(String projectId, String region, String clusterId, int port) {
    return String.format("bootstrap.%s.%s.managedkafka.%s.cloud.goog:%d", 
                        clusterId, region, projectId, port);
  }
