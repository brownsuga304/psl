 ...
 *
 * <p>This test demonstrates the cursor and topic stats functionality: - Committing and reading
 * offsets (cursor management) - Getting earliest/latest offsets (topic stats) - Computing backlog
 * estimates
 *
 * <p>Run with: ./test-managed-kafka-stats.sh BOOTSTRAP_SERVER TOPIC_NAME [NUM_MESSAGES]
 *
 * <p>Example: ./test-managed-kafka-stats.sh
 * bootstrap.test-kafka.us-central1.managedkafka.myproject.cloud.goog:9092 test-topic 10
 */
public class ManagedKafkaStatsTest {
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (bootstrapServers == null || topicName == null) {
      System.out.println(
          "Usage: ManagedKafkaStatsTest <BOOTSTRAP_SERVER> <TOPIC_NAME> [NUM_MESSAGES]");
      System.out.println();
    ...
    try {
      ...
      Offset earliestOffset =
          statsClient.getEarliestOffset(topicName, partition).get(30, TimeUnit.SECONDS);
      System.out.println("  Earliest offset: " + earliestOffset.value());

      // Get latest offset
      System.out.println("Getting latest offset for partition 0...");
      Offset latestOffset =
          statsClient.getLatestOffset(topicName, partition).get(30, TimeUnit.SECONDS);
      ...
      Cursor headCursor =
          statsClient.computeHeadCursor(topicPath, partition).get(30, TimeUnit.SECONDS);
      ...
      ComputeMessageStatsResponse stats =
          statsClient
              .computeMessageStats(topicPath, partition, earliestOffset, latestOffset)
      ...
      ComputeMessageStatsResponse newStats =
          statsClient
              .computeMessageStats(topicPath, partition, latestOffset, newLatestOffset)
              .get(30, TimeUnit.SECONDS);
      ...
      List<PartitionCursor> cursors =
          cursorClient.readCommittedOffsets(subscriptionPath, topicName).get(30, TimeUnit.SECONDS);
      if (cursors.isEmpty()) {
        System.out.println("  No committed offsets found (new consumer group)");
      } else {
        for (PartitionCursor pc : cursors) {
          System.out.println(
              "  Partition " + pc.getPartition() + ": offset " + pc.getCursor().getOffset());
        }
      }
      ...
      cursorClient
          .commitOffset(subscriptionPath, topicName, partition, commitOffset)
          .get(30, TimeUnit.SECONDS);
      ...
      Cursor committedCursor =
          cursorClient
              .getCommittedOffset(subscriptionPath, topicName, partition)
              .get(30, TimeUnit.SECONDS);
      ...
      System.out.println(
          "Setting consumer offset to " + behindOffset.value() + " (simulating lag)...");

      Map<Partition, Offset> resetOffsets = new HashMap<>();
      resetOffsets.put(partition, behindOffset);
      cursorClient
          .resetOffsets(subscriptionPath, topicName, resetOffsets)
          .get(30, TimeUnit.SECONDS);
